...
class Unet(object):
    ...
    @staticmethod
    def _unwrap_main_logit(prediction):
    ...
    def detect_image(self, image, count=False, name_classes=None):
        ...
        with torch.no_grad():
            ...
            #   图片传入网络进行预测
            #---------------------------------------------------#
            pr = self._unwrap_main_logit(self.net(images))
            print("Raw output shape:", pr.shape, "Min:", pr.min().item(), "Max:", pr.max().item())
            #---------------------------------------------------#
            #   取出每一个像素点的种类
            #---------------------------------------------------#
            pr = F.softmax(pr.permute(1,2,0),dim = -1).cpu().numpy()
            print("Softmax output shape:", pr.shape, "Min:", pr.min(), "Max:", pr.max())
            # 分别保存两个通道为 .txt 文件
            channel_0 = pr[:, :, 0]  # 提取第一个通道
            channel_1 = pr[:, :, 1]  # 提取第二个通道
            # 保存为 .txt 文件
            np.savetxt('channel_0.txt', channel_0, fmt='%.6f', delimiter=',')
            np.savetxt('channel_1.txt', channel_1, fmt='%.6f', delimiter=',')
            # 计算通道 1 的最大值及其位置
            max_value_channel_1 = np.max(channel_1)
            max_index = np.unravel_index(np.argmax(channel_1), channel_1.shape)
            print(f"Channel 1 Max Value: {max_value_channel_1:.6f}")
            print(f"Max Value Position (row, col): {max_index}")
            #--------------------------------------#
            #   将灰条部分截取掉
            #--------------------------------------#
